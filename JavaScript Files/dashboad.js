...
//     // 1. Pehle authenticated user get karein
//     const { data: { user }, error: authError } = await supabase.auth.getUser();

//     if (authError || !user) {
//         console.log("User not logged in");
//         window.location.href = "./login/login.html"; // Redirect agar login nahi hai
//         return;
//     }

//     // 2. Ab specific user ka role fetch karein (.eq filter ke sath)
//     const { data: userData, error: dbError } = await supabase
//         .from('FullStack-Users')
//         .select('role, email') // Dono columns ek hi string mein comma ke sath
//         .eq('id', user.id)    // <--- Ye filter lagana lazmi hai
//         .single();            // Ab .single() sahi kaam karega kyunki ab sirf 1 row aayegi
...
// // dashboard.js (Bilkul shuru mein)
// async function protectDashboard() {
//     const { data: { user }, error: authError } = await supabase.auth.getUser();

//     // 1. Agar logged in nahi hai to login page par bhej dein
//     if (!user || authError) {
